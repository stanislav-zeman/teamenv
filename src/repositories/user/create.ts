...
export async function createProjectMember(data: CreateProjectMemberData): Promise<Result<ProjectUser>> {
  try {
    ...
    if (userRole.unwrap() !== "OWNER") {
      if (!hasAtLeastRole(userRole.unwrap(), "MAINTAINER")) {
        return Result.err(new Error("Only maintainers and above can invite users to project!"));
      }
      if (data.role === "MAINTAINER") {
        return Result.err(new Error("Only owners can invite users with maintainer role!"));
      }
    }
